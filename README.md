...
- **문장 듣기 (TTS)** — 선택한 문장을 영어 음성으로 낭독. 브라우저 내장
  음성 합성(Web Speech API)을 사용하므로 키·비용·네트워크 전송이 없고,
  속도 조절(0.7×–1.15×)과 다시 듣기를 지원
- **연속 듣기 (오디오북 모드)** — 현재 페이지부터 문장 단위로 이어서 낭독.
  문장 넘기기·일시정지·속도 조절을 지원하고 페이지도 자동으로 넘어감
- **단어 탭 사전** — 단어를 클릭(탭)하면 발음(IPA)·품사·문맥 속 뜻 카드가
  뜨고, 🔊 발음 듣기와 단어장 저장 버튼 제공
- **단어장** — 모르는 단어를 뜻·문맥과 함께 브라우저에 저장. 발음 다시 듣기,
  삭제, CSV 내보내기 지원
